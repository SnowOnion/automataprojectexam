...
/**
 * Create by: huangcd Date: 2009-12-10 Time: 12:45:17
 */
@SuppressWarnings("unchecked")
public class DefaultXMLAutomatonReader implements
        XMLAutomatonReader<String, State>
{
    ...
    private DFA<String> parseDFA(Element root)
    {
        ...
        // parse symbols
        List<Element> xmlSymbols = root.element(INPUT_SYMBOLS).elements(
                INPUT_SYMBOL);
        ...
        // parse transitions
        List<Element> xmlTransitions = root.element(TRANSITIONS).elements(
                TRANSITION);
        for (Element xmlTransition : xmlTransitions)
        {
            String fromID = xmlTransition.element(FROM_STATE).attributeValue(
                    STATE_ID);
            DFAState fromState = dfa.getStateByID(fromID);
            List<Element> conditions = xmlTransition.element(CONDITIONS)
                    .elements();
            List<Element> xmlToStates = xmlTransition.element(TO_STATES)
                    .elements();
    ...
    public DFA<String> readDFA(File file) throws Exception
    {
        ...
        {
            log.log(Level.SEVERE, "exception occurs while reading \"" + file
                    + "\"", e);
            throw e;
        }
    }
    ...
    public DFA<String> readDFA(String content) throws Exception
    {
        ...
        {
            log.log(Level.SEVERE, "exception occurs while reading \"" + content
                    + "\"", e);
            throw e;
        }
    }
    ...
    public DFA<String> readDFA(InputStream in) throws Exception
    {
        ...
        {
            log.log(Level.SEVERE, "exception occurs while reading \"" + in
                    + "\"", e);
            throw e;
        }
    }
    ...
    public DFA<String> readDFA(URL url) throws Exception
    {
        ...
        {
            log.log(Level.SEVERE, "exception occurs while reading \"" + url
                    + "\"", e);
            throw e;
        }
    }
    ...
    public FiniteAutomaton readFiniteAutomaton(File file) throws Exception
    {
        try
        {
            ...
            {
                String message = "cannot convert a \"" + type
                        + "\" to a FiniteAutomaton";
                log.log(Level.SEVERE, message, this);
                throw new UnconvertableException(message);
            }
        }
